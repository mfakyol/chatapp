...
export function MessageInfo({
  ...
  return (
    <div className="mt-1 w-56 rounded-md bg-(--bg-surface) px-3 py-2 text-xs text-(--text-normal) shadow-lg">
      <p className="text-(--text-muted)">{t('chat.detailSent', { time: sentAt })}</p>

      {isGroup ? (
        <>
          {readers.length > 0 && (
            <>
              <p className="mt-2 flex items-center gap-1 font-semibold text-(--tick)">
          ...
          {deliveredOnly.length > 0 && (
            <>
              <p className="mt-2 flex items-center gap-1 font-semibold text-(--text-muted)">
          ...
          {readers.length === 0 && deliveredOnly.length === 0 && (
            <p className="mt-1 text-(--text-muted)">{t('chat.detailNotSeen')}</p>
          )}
        </>
      ) : readers.length > 0 ? (
        <p className="mt-1 flex items-center gap-1">
          <IconChecks size={14} className="text-(--tick)" />
          {t('chat.detailSeen', {
